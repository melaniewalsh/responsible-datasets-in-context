...
The specific topic the “Border Women Literature & Feminist Cartographies” course focused on gender violence and the _feminicidios _in the transborder region of Ciudad Juárez, Mexico, El Paso, Texas, and Santa Teresa, New Mexico. The course first focused on a localized perspective in this transborder region before shifting to a broader U.S. and global perspective. 
...
//| echo: false
//| output: false
tracing_femenicide_data = d3.csv("https://raw.githubusercontent.com/melaniewalsh/responsible-datasets-in-context/main/datasets/gender-violence/Tracing_Feminicides_in_El_Paso,_Texas_Using_Newsbank's_Hispanic_American_Newspaper_Collection(Violencia_Violence_Feminicida_Fem).csv", d3.autoType)
```

``` {ojs}
//| echo: false
viewof search_femenicide_data = Inputs.search(tracing_femenicide_data, {
  placeholder: "Search"
})
```
...
Inputs.table(search_femenicide_data, {
  layout: "fixed",
  rows: 50,
  sort: "Year",
  reverse: true,
...
<-- Put both spanish and English data set for this one. -->
...
//| echo: false
//| output: false
geotestimonios_literary_data = d3.csv("https://raw.githubusercontent.com/melaniewalsh/responsible-datasets-in-context/main/datasets/gender-violence/Geographical_Metadata_Women_Hollering&Mystery_of_Survival(Woman_Hollering_Creek).csv", d3.autoType)
```

``` {ojs}
//| echo: false
viewof search_geotestimonios_literary_data = Inputs.search(geotestimonios_literary_data, {
  placeholder: "Search"
})
```
...
Inputs.table(search_geotestimonios_literary_data, {
  layout: "fixed",
  rows: 50,
  sort: "Year",
  reverse: true,
...
//| echo: false
//| output: false
anti_gender_art_data = d3.csv("https://raw.githubusercontent.com/melaniewalsh/responsible-datasets-in-context/main/datasets/gender-violence/Art_of_Anti_Gender_Violence_and_Femincides_Database.csv", d3.autoType)
```

``` {ojs}
//| echo: false
viewof search_anti_gender_art_data = Inputs.search(anti_gender_art_data, {
  placeholder: "Search"
})
```
...
Inputs.table(anti_gender_art_data, {
  layout: "fixed",
  rows: 50,
  sort: "ID NO.",
  reverse: true,
