...
<br>
<!-- Importing from https://observablehq.com/d/4a39c8f7f8be9561 -->

```{ojs}
import {viewof dataSummaryView, viewof selectedColumns, viewof dataUrl, viewof dataSet, tableContainer, table, tableStyles} from "4a39c8f7f8be9561"
```
...
def print_rankings(d, col_name):
    ...
    if (math.isnan(rank_B)):
      points_moved = 501
      d["html_output"] = f'<span style="color:black"> ● {title}</span>'
    else:
      if rank_B > int(rank_A):
          points_moved = rank_B - rank_A
          d["html_output"] = f'<span style="color:red"> ▼ -{int(points_moved)} {title}</span>'
      elif rank_B < rank_A:
          points_moved = rank_A - rank_B
          d["html_output"] = f'<span style="color:green"> ▲ +{int(points_moved)} {title}</span>'
      else:
          d["html_output"] = f'<span style="color:black"> ● {title}</span>'
